...
include_once $dir.'validlimit.php';
...
function write_valid_limit($dir,$nmlmtstr,$iplmtstr){
	...
	$vldata = "<?php\n\n\$nmlimit = {$nmlmtlist};\n\$iplimit = {$iplmtlist};\n\n?>";
	if($fp = fopen("{$dir}validlimit.php", 'w')) {
		if(flock($fp,LOCK_EX)) {
			fwrite($fp, $vldata);
		} else {
...
echo <<<EOT
<form method="post" name="validlimit" onsubmit="admin.php">
<input type="hidden" name="mode" value="validlimitmng">
<input type="hidden" name="command" value="validlimitmng">
<input type="hidden" name="write" value="1">
<div>输入要屏蔽的用户名和IP段，用|隔开。</div>
<div>用户名屏蔽：<br><textarea name="postnmlmt" style="width:450;height:150">$nmlmtlist</textarea></div><br>
<div>IP段屏蔽：<br><textarea name="postiplmt" style="width:450;height:150">$iplmtlist</textarea></div>
