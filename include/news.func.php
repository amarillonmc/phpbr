...
function  parse_news($start = 1, $range = 0 , $file = '') {
	...
	for($i = $start;$i <= $r;$i++) {
		...
		if($news == 'newgame') {
			$newsinfo .= "<li>{$hour}时{$min}分{$sec}秒，<span class=\"red\">第{$a}回 常磐祭开始了</span><br>\n";
		} elseif($news == 'gameover') {
			$newsinfo .= "<li>{$hour}时{$min}分{$sec}秒，<span class=\"red\">第{$a}回 常磐祭结束了</span><br>\n";
		} elseif($news == 'newpc') {
			...
			$newsinfo .= "<li>{$hour}时{$min}分{$sec}秒，<span class=\"red\">游戏进入 连斗 ！</span><br>\n";
		} elseif($news == 'end0') {
			...
			$newsinfo .= "<li>{$hour}时{$min}分{$sec}秒，<span class=\"yellow\">$a 打倒了举办者，游戏紧急结束</span><br>\n";
		} elseif($news == 'end4') {
			$newsinfo .= "<li>{$hour}时{$min}分{$sec}秒，<span class=\"red\">无人参加，游戏自动结束</span><br>\n";
		} elseif($news == 'end5') {
			$newsinfo .= "<li>{$hour}时{$min}分{$sec}秒，<span class=\"red\">$a 无意中引爆了核弹，所有参赛者全灭！</span><br>\n";
			...
			if($news == 'death11') {
				...
				$newsinfo .= "<li>{$hour}时{$min}分{$sec}秒，<span class=\"yellow\">$a</span> 因 <span class=\"red\">hack失败</span> 死亡";
			} elseif($news == 'death15') {
				...
				$newsinfo .= "<li>{$hour}时{$min}分{$sec}秒，<span class=\"yellow\">$a</span> 因 <span class=\"yellow\">$d</span> 意外死亡";
			} elseif($news == 'death30') {
				$newsinfo .= "<li>{$hour}时{$min}分{$sec}秒，<span class=\"yellow\">$a</span> 因 在RP很低的情况下做了不该做的事情而欢乐地领到便当！";
			} elseif($news == 'death31'){
			...
			$newsinfo .= "<li>{$hour}时{$min}分{$sec}秒，<span class=\"yellow\">$a</span> 被 <span class=\"yellow\">神北 小毬 许愿复活</span><br>\n";
		} elseif($news == 'delcp') {
